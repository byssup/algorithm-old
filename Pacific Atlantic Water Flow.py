...
class Solution(object):
    def pacificAtlantic(self, matrix):
        ...
        pacific = set([(0, i) for i in range(width)]).union(set([(i, 0) for i in range(1, height - 1)]))
        atlantic = set([(height - 1, i) for i in range(width)]).union([(i, height - 1) for i in range(1, height - 1)])
